...
#notes-sub([Ортогональное дополнение], pgbreak: true)
#def(
  [Ортогональное дополнение],
  [
    $L^perp$ называется ортогональным дополнением $L$ (подпространства евклидова пространства $E$), если
    $
      L^perp = {x in E : x perp L}
    $
    Свойсва:
    + $L^perp$ --- подпространство $E$
    + $E = L plus.o L^perp$
...
#theorem(
  [
    Докажем свойство 3 из предыдущего определения:
    $
      E = L plus.o L^perp
    $
  ],
  [
    Рассмотрим $L subset.eq E$. Пусть $S = {e_1, e_2, dots, e_k}$ --- ортонормированный базис $L$ (его существование очевидно). Дополним $S$ до ОНБ (ортонормированного базиса) $E$ (дополним произвольными и ортогонализируем через Грама-Шмидта):
    $
      S_E = {e_1, e_2, dots, e_k, e_(k+1), dots, e_n}
    $
    Пусть $x in E$, тогда
    $
      x = limits(sum)_(i=1)^k x_i e_i + limits(sum)_(i=k+1)^n x_i e_i
    $
    очевидно, $L = chevron e_1, dots, e_k chevron.r$, обозначим $L_1 := chevron e_(k+1), dots, e_n chevron.r$

    Заметим, что $L_1 subset.eq L^perp$, т.к. $(forall x in L_1)(x perp L)$, покажем включение в другую сторону. Выберем $y in E$ с координатами $y_1, dots, y_n$, пусть $y perp L$, тогда $y perp {e_1, dots, e_k}$ (т.к. они принадлежат $L$), а значит, если $j in [1, k]$, то
    $
      & (y, e_j) = 0, "откуда" \
      & (limits(sum)_(i=1)^k y_i e_i + underbracket(limits(sum)_(i=k+1)^n y_i e_i, 0), e_j) = 0 \
      & y_j underbracket((e_j, e_j), 1) = 0 "(остальное зануляется из-за ортогональности)"
    $
    то есть, $y = {0, 0, dots, e_(k+1), dots, e_n} <==> L^perp subset.eq L$, а значит $L^perp = L_1$
  ],
)

#notes-sub([Ортогональная проекция], pgbreak: true)

Рассмотрим $L, L^perp : L plus.o L^perp = E$. Пусть $S = {a_1, a_2, dots, a_k}$ --- ортонормированная система векторов $E$; $b in E$, тогда
$
  alpha_i = (b, a_i)
$
называются коэффициентами Фурье вектора $b$ относительно системы векторов.

#def(
  [Ортогональная проекция],
  [
    ортогональной проекцией вектора $a$ на подпространство $L$ ($"pr"_L (a) = p_a$) называется вектор-проекция $p$, такая что
    $
      a = p_a + r, "где" r perp L, p_a - "вектор-проекция"
    $
    как правило обозначают $r = "ort"_L (a)$
  ],
)
#theorem(
  [
    Пусть $e_1, e_2, dots, e_k$ --- ОНБ евклидова пространства $L subset.eq E$, $a in E$ --- произвольный вектор, тогда
    $
      "pr"_L (a) = limits(sum)_(i=1)^k alpha_i e_i, "где" alpha_i - "коэффициенты Фурье"
    $
  ],
  [
    Докажем, что вектор с такими координатами имеет такое разложение, пусть:
    $
      & p = limits(sum)_(i = 1)^k (a, e_i)e_i
    $
    очевидно, $p in L$, т.к. является линейной комбинацией $e_i$. Обозначим $a':= a - p$, покажем, что $a' perp L$:
    $
      & (a - p, e_j) = (a - limits(sum)_(i = 1)^k (a, e_i)e_i, e_j) = \
      & (a, e_j) - (limits(sum)_(i = 1)^k (a, e_i)e_i, e_j) = \
      & (a, e_j) - limits(sum)_(i = 1)^k ((a, e_i)e_i, e_j) = (a, e_j) - underbracket(limits(sum)_(i = 1)^k (a, e_i)(e_i, e_j), (a, e_j)(e_j, e_j)) = \
      & (a, e_j) - (a, e_j)underbracket((e_j, e_j), norm(e_j)=1) = (a, e_j) - (a, e_j) = 0
    $
    то есть
    $
      a = p + a', "причем" p in L, a' perp L
    $
    а это в точности определение ортогональной проекции
  ],
)
...
#fun-fact-headered(
  ...
  [
    Заключается в нахождении компонент в подпространстве $L$ и его ортогональном дополнении.
  ],
)
Для решения задачи рассмотрим $E = L + L^perp$, $S = {e_1, dots, e_n}$ --- ОНБ пространства $E$, где $L = chevron e_1, dots, e_k chevron.r, L^perp = chevron e_(k+1), dots, e_n chevron.r$. По доказанному
$
  "pr"_L (x) = (alpha_1, dots, alpha_k, 0, dots, 0)
$
ну и тогда
$
  "ort"_L (x) = x - "pr"_L (x)
$
Второй способ решения этой задачи состоит в решении СЛАУ, составляемой следующим образом:

Пусть $x in E, x = "pr"_L (x) + "ort"_L (x)$, $L = chevron a_1, a_2, dots, a_k chevron.r$ ($a_i$ --- ЛНЗ). По определению
$
  "pr"_L (x) = limits(sum)_(i = 1)^k alpha_i a_i
$
тогда
$
  "ort"_L (x) = x - "pr"_L (x)
$
запишем условие ортогональности $"ort"_L (x) perp L$:
$
  & (x - "pr"_L (x), a_j) = 0 \
  & (x, a_j) - (limits(sum)_(i = 1)^k alpha_i a_i, a_j) = \
  & (x, a_j) - limits(sum)_(i = 1)^k (alpha_i a_i, a_j) = (x, a_j) - limits(sum)_(i = 1)^k alpha_i (a_i, a_j)
$
перепишем в форме системы уравнений:
$
  cases(
    (x, a_1) - alpha_1 (a_1, a_1) - alpha_2 (a_1, a_2) - dots - alpha_k (a_1, a_k),
    (x, a_2) - alpha_1 (a_2, a_1) - alpha_2 (a_2, a_2) - dots - alpha_k (a_2, a_k),
    dots,
    (x, a_k) - alpha_1 (a_k, a_1) - alpha_2 (a_k, a_2) - dots - alpha_k (a_k, a_k),
  )
$
или
$
  cases(
    alpha_1 (a_1, a_1) + alpha_2 (a_1, a_2) + dots + alpha_k (a_1, a_k) = (x , a_1),
    alpha_2 (a_1, a_2) + alpha_2 (a_2, a_2) + dots + alpha_k (a_2, a_k) = (x , a_2),
    dots,
    alpha_1 (a_k, a_1) + alpha_2 (a_k, a_2) + dots + alpha_k (a_k, a_k) = (x , a_k),
  )
$
ну и матричная форма
$
  underbracket(
    mat(
      (a_1, a_1), (a_1, a_2), dots.c, (a_1, a_k);
      (a_2, a_1), (a_2, a_2), dots.c, (a_2, a_k);
      dots;
      (a_k, a_1), (a_k, a_2), dots.c, (a_k, a_k);
    ), "это если что матрица грама для" a_i
  ) dot vec(alpha_1, alpha_2, dots, alpha_k)= vec((x, a_1), (x, a_2), dots, (x, a_k))
$
Здесь неизвестными выступают коэффициенты вектора $"pr"_L (a)$ в базисе $(a_1, a_2, dots, a_k)$, а найдя этот вектор несложно найти и $"ort"_L (a)$
