...
> 
> Типовые шаблонные параметры задаются при помощи ключевых слов `typename` и `class`:
> 
> ```cpp
> template<typename T>
> // template<class T>
...
Шаблоны методов не могут быть виртуальными или перегружать родительские виртуальные методы, а также определять конструкторы копирования или деструкторы. 
...
Начиная с C++17, пользователь может самостоятельно задать правила вывода параметров шаблонного класса, что может быть полезно, когда автоматический вывод невозможен или не удовлетворяет нашей семантике:
...
// теперь при вызове конструктора Wrapper от строкового литерала его тип
// определится как std::string
```
...
Если мы хотим воздействовать непосредственно на каждый из элементов, то до C++17 нам пришлось бы прибегать к рекурсии:
...
В C++17 же был добавлен механизм *выражений свёртки* (*fold expressions*), который позволяет объединить раскрытие пакета параметров с применением унарной или бинарной операции. Его синтаксис имеет следующие вариации: 
...
Легко видеть, что они ощутимо упрощают решение нашей ихсодной задачи:
...
Чистые выражения свёртки на первый взгляд имеют один недостаток: `op` обязан представлять собой оператор, то есть, например, следующий код невалиден:
...
Решение этой проблемы носит название *Comma fold pattern* и пользуется тем фактом, что выражения свёртки поддерживают оператор запятая. По умолчанию он вычисляет значение слева, отбрасывает результат и возвращает результат вычисления выражения справа, то есть в случае пакета аргументов в качестве побочного эффекта он всегда применит заданную операцию к каждому из них, что нам и нужно. В качестве этой операции может выступать не только функция, но и любое другое выражение; например, использование этого паттерна позволяет добавить вывод разделителя в нашу реализацию `Print`:
...
Вариадические шаблоны используются в реализации таких классов, как `std::tuple` и `std::variant`. Со вторым, в частности, связана ещё одна маленькая, но полезная идея, которую стоит упомянуть.

`std::variant` — это типобезопасный юнион. Мы верим, что читатель в состоянии обратиться к cppreference, и потому не будем в подробностях описывать его интерфейс. Важно лишь сказать, что для вызова функции от сохраняемого в нём значения в современном C++ используется `std::visit`, который предоставляет более чистый, безопасный и быстрый интерфейс сравнительно ручного взятия значения. 

Поскольку `std::variant` может содержать значение одного из произвольного количества типов, нам очевидно хотелось бы, чтобы функция, передаваемая в `std::visit`, имела перегрузку для каждого или хотя бы для некоторых из них, что проблематично при работе с лямбдами. Решением этой задачи является *Overload pattern*, который использует вариадические шаблоны и выведение типа для создания единого класса, который объединяет в себе функционал нескольких функторов:
...
int main() {
	...
	std::visit(Overload{
		[](int x) { std::cout << "Integer\n"; },
		[](const std::string& s) { std::cout << "String\n"; },
		[](float x) { std::cout << "Floating point number\n"; }
	}, var);
}
```
...
8. cppreference.com — Fold expressions (https://en.cppreference.com/cpp/language/fold)
