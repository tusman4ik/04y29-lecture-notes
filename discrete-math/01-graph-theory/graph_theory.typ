...
#def(
  [Матрица смежности],
  [
    для графа с $n$ вершинами матрицей смежности будет матрица размера $n times n$, элементы которой задаются по следующим правилам
    $
...
#def(
  [Изоморфные графы],
  [
    $G_1 = pair(V_1, E_1)$ и $G_2 = pair(V_2, E_2)$, для которых существует биекция $phi : V_1 -> V_2$, сохраняющая смежность:
    $
